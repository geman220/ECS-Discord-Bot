...
class VerifyModal(discord.ui.Modal):
    ...
    order_id = discord.ui.TextInput(
        label="Order ID", placeholder="Enter your order ID number here"
    )
        ...
        if response_data:
            ...
            if order_status in ["processing", "completed"]:
                ...
                role = discord.utils.get(
                    interaction.guild.roles, name=current_membership_role
                )
                ...
                if role:
                    ...
                    await interaction.response.send_message(
                        f"{current_membership_role} role not found.", ephemeral=True
                    )
            else:
                await interaction.response.send_message(
                    "Invalid order number or order status not eligible.", ephemeral=True
                )
        else:
            await interaction.response.send_message(
