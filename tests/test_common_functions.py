...
async def test_create_event_if_necessary(monkeypatch):
    ...
    monkeypatch.setattr('discord.Guild.fetch_scheduled_events', mock_fetch_scheduled_events)
    monkeypatch.setattr('discord.Guild.create_scheduled_event', mock_create_scheduled_event)
...
async def test_check_existing_threads(monkeypatch):
    ...
    # Define the mock function with logic to check for channel name
    def mock_get_discord_utils_get(*args, **kwargs):
        if kwargs.get('name') == "test_channel":
            return channel
        return None

    # Monkeypatch the discord.utils.get function
...
async def test_generate_flight_search_url(monkeypatch):
    ...
    monkeypatch.setattr('common.get_airport_code_for_team', mock_get_airport_code_for_team)
    monkeypatch.setattr('common.fetch_serpapi_flight_data', mock_fetch_serpapi_flight_data)

    interaction = AsyncMock()
    result = await generate_flight_search_url(interaction, "SEA", "Team XYZ", datetime.date(2024, 3, 1), datetime.date(2024, 3, 2))

    # Assert that the result contains the expected URL
