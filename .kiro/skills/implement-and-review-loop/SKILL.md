...
The system supports a maximum of 4 concurrent subagents per invocation. When a workflow step requires more than 4 subagents, batch them: run the first 4, wait for all to complete, then run the remaining subagents. This applies to any skill invoked by this loop (e.g., `review-code` uses 5 review subagents in 2 batches (4 + 1) to respect the concurrency limit).
...
**⚠️ MUST USE 4-AGENT PARALLEL REVIEW via `use_subagent`. Inline review (doing all 4 categories yourself) is NOT acceptable unless ALL of the following are true: (1) `use_subagent ListAgents` was attempted, (2) the required agents are not discoverable, AND (3) you explicitly told the user "Subagents unavailable, falling back to inline review." If you default to inline without attempting subagents, you are violating this skill.**
...
1. Steps 1–3 execute normally (gather changes → read code → invoke 5 review subagents in 2 batches (4 parallel + 1)).
