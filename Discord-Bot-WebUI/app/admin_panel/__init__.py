...
def _build_admin_search_index():
    ...
    items = [
        ...
        {'name': 'Compose a Message', 'category': 'Comms', 'description': 'Write once — send via app alert, push, email, Discord DM, or SMS',
         'keywords': ['compose', 'multi-channel', 'send message', 'all channels', 'composer'],
         'url': _safe_url('admin_panel.message_composer'), 'icon': 'ti-pencil-bolt'},
        {'name': 'Push Notifications', 'category': 'Comms', 'description': 'Send an alert to phones with the ECS app',
         'keywords': ['push', 'alerts', 'mobile notifications', 'web push'],
         'url': _safe_url('admin_panel.push_notifications'), 'icon': 'ti-bell'},
        {'name': 'Homepage Announcements', 'category': 'Comms', 'description': 'Banner on the member homepage — does not notify anyone',
         'keywords': ['news', 'banner', 'announcement', 'notice'],
         'url': _safe_url('admin_panel.announcements'), 'icon': 'ti-speakerphone'},
        {'name': 'Match RSVP Posts', 'category': 'Comms', 'description': 'Scheduled RSVP embeds posted to team Discord channels',
         'keywords': ['queue', 'scheduled', 'rsvp', 'availability', 'scheduled messages'],
         'url': _safe_url('admin_panel.scheduled_messages_queue'), 'icon': 'ti-clock'},
        {'name': 'Push — Scheduled & History', 'category': 'Comms', 'description': 'Schedule pushes and review how past sends performed',
         'keywords': ['campaign', 'push campaigns', 'outreach', 'bulk messaging', 'schedule push'],
         'url': _safe_url('admin_panel.campaigns_list'), 'icon': 'ti-broadcast'},
        {'name': 'Email Blasts', 'category': 'Comms', 'description': 'One-time emails to a group of members',
         'keywords': ['email', 'mass email', 'newsletter', 'bulk email', 'broadcast'],
         'url': _safe_url('admin_panel.email_broadcasts_list'), 'icon': 'ti-mail-forward'},
        {'name': 'Email Layouts', 'category': 'Comms', 'description': 'Reusable HTML wrappers for email blasts',
         'keywords': ['email design', 'html email', 'template editor', 'email templates'],
         'url': _safe_url('admin_panel.email_templates_list'), 'icon': 'ti-template'},
        {'name': 'Audiences', 'category': 'Comms', 'description': 'Saved recipient groups for push targeting',
         'keywords': ['groups', 'recipients', 'mailing lists', 'segments', 'notification groups'],
         'url': _safe_url('admin_panel.notification_groups_list'), 'icon': 'ti-users-group'},
        {'name': 'In-App Messaging', 'category': 'Comms', 'description': 'Settings for the on-site player/coach/admin DM system',
         'keywords': ['config', 'dm', 'direct message settings', 'messaging settings'],
         'url': _safe_url('admin_panel.messaging_settings'), 'icon': 'ti-settings'},

        # --- Users ---
        # The standalone Users / Approvals / Waitlist / Quick Profiles / Analytics pages
        # were retired from nav — each is now a TAB of the Members command center. Their
        # search entries keep the OLD names (that's what admins type) but point at the
        # matching hub tab. The legacy routes stay live as a burn-in fallback.
        {'name': 'Members', 'category': 'Users', 'description': 'Member worklist and person-360 hub — search, approve, place, and manage everyone',
         'keywords': ['members', 'people', 'roster', 'worklist', 'person', 'member hub', 'command center'],
         'url': _safe_url('admin_panel.members_worklist'), 'icon': 'ti-id-badge-2'},
        ...
        {'name': 'Waitlist', 'category': 'Users', 'description': 'Manage user registration waitlist',
         ...
         'url': _safe_url('admin_panel.members_worklist', tab='waitlist'), 'icon': 'ti-user-plus'},
        {'name': 'Roles', 'category': 'Users', 'description': 'Create and manage user roles and permissions',
         ...
         'url': _safe_url('admin_panel.access_control', tab='roles'), 'icon': 'ti-shield'},
        {'name': 'Quick Profiles', 'category': 'Users', 'description': 'Manage quick profile entries for tryouts',
         ...
         'url': _safe_url('admin_panel.members_worklist', tab='quick'), 'icon': 'ti-id-badge'},
        {'name': 'User Analytics', 'category': 'Users', 'description': 'User registration and activity analytics',
         ...
         'url': _safe_url('admin_panel.members_worklist', tab='analytics'), 'icon': 'ti-chart-line'},
        ...
        # The scattered monitoring pages were retired from nav — each is now a TAB of the
        # System Command Center. Their search entries keep the OLD names (that's what
        # admins type) but point at the matching hub tab. Where a legacy page still holds
        # unique controls (task monitor, docker, security add-ban, API keys, integrity
        # resolve), the hub deep-links to it and the route stays live.
        {'name': 'System Center', 'category': 'System', 'subcategory': 'Command Center',
         'description': 'Unified operations surface — health, services, performance, jobs, data, security, logs, API',
         'keywords': ['system', 'ops', 'operations', 'command center', 'monitoring', 'health', 'status', 'hub'],
         'url': _safe_url('admin_panel.system_center'), 'icon': 'ti-heartbeat'},
        {'name': 'System Health', 'category': 'System', 'subcategory': 'Command Center',
         'description': 'System health, service status, performance metrics, and diagnostics',
         'keywords': ['health', 'uptime', 'status', 'services', 'monitoring'],
         'url': _safe_url('admin_panel.system_center', tab='overview'), 'icon': 'ti-heartbeat'},
        {'name': 'Services', 'category': 'System', 'subcategory': 'Command Center',
         'description': 'Per-service status board — Discord, database, Redis, Celery, push, email, Twilio, Docker',
         'keywords': ['services', 'discord', 'redis', 'celery', 'email', 'push', 'twilio', 'docker', 'containers'],
         'url': _safe_url('admin_panel.system_center', tab='services'), 'icon': 'ti-server-2'},
        {'name': 'System Performance', 'category': 'System', 'subcategory': 'Command Center',
         'description': 'CPU, memory, disk, and request/query performance metrics',
         'keywords': ['performance', 'cpu', 'memory', 'disk', 'slow', 'latency', 'load'],
         'url': _safe_url('admin_panel.system_center', tab='perf'), 'icon': 'ti-gauge'},
        {'name': 'Task Monitor', 'category': 'System', 'subcategory': 'Command Center',
         'description': 'Monitor background task execution and Celery workers',
         'keywords': ['celery', 'tasks', 'background jobs', 'workers', 'queue', 'jobs', 'beat', 'schedule'],
         'url': _safe_url('admin_panel.system_center', tab='jobs'), 'icon': 'ti-list-check'},
        {'name': 'Cache & Redis', 'category': 'System', 'subcategory': 'Command Center',
         'description': 'Cache operations, Redis connection pool, database and memory management',
         'keywords': ['cache', 'clear cache', 'redis', 'flush', 'memory store', 'connection pool',
                      'database monitor', 'db', 'draft cache'],
         'url': _safe_url('admin_panel.system_center', tab='data'), 'icon': 'ti-database'},
        {'name': 'Security Dashboard', 'category': 'System', 'subcategory': 'Command Center',
         'description': 'Security overview, IP bans, rate limiting, and threat monitoring',
         'keywords': ['security', 'login attempts', 'threats', 'ip ban', 'rate limit', 'authentication'],
         'url': _safe_url('admin_panel.system_center', tab='security'), 'icon': 'ti-shield-check'},
        {'name': 'System Logs', 'category': 'System', 'subcategory': 'Command Center',
         'description': 'Application logs, container logs, and the admin action audit trail',
         'keywords': ['logs', 'log file', 'errors', 'stack trace', 'container logs'],
         'url': _safe_url('admin_panel.system_center', tab='logs'), 'icon': 'ti-file-text'},
        {'name': 'Audit Logs', 'category': 'System', 'subcategory': 'Command Center',
         'description': 'View admin action audit trail and history',
         'keywords': ['logs', 'history', 'audit', 'trail', 'actions', 'who changed'],
         'url': _safe_url('admin_panel.system_center', tab='logs', src='audit'), 'icon': 'ti-file-text'},
        {'name': 'API Management', 'category': 'System', 'subcategory': 'Command Center',
         'description': 'API traffic, endpoints, keys, and rate limits',
         'keywords': ['api keys', 'tokens', 'rate limit', 'endpoints', 'api traffic', 'requests'],
         'url': _safe_url('admin_panel.system_center', tab='api'), 'icon': 'ti-api'},
        ...
        # --- System: Feedback ---
        {'name': 'Feedback', 'category': 'System', 'subcategory': 'Feedback',
        ...
        {'name': 'Feature Toggles', 'category': 'System', 'subcategory': 'Configuration',
        ...
        {'name': 'Theme Settings', 'category': 'System', 'subcategory': 'Configuration',
