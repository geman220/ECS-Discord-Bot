                    ...
                    .then(data => {
                        if (data.success) {
                            window.Swal.fire({
                                ...
                                html: `<p>${data.message}</p><p class="text-muted small mt-2">${data.synced_count || 0} templates synchronized</p>`,
                                icon: 'success'
                            });
                        } else {
                ...
                didOpen: () => {
                    ...
                    .then(data => {
                        if (data.success) {
                            window.Swal.fire({
                                title: 'Alert Sent!',
                                html: `<p>${data.message}</p><p class="text-muted small mt-2">${data.sent_count || 0} notifications sent</p>`,
                                icon: 'warning'
                            });
                        } else {
    ...
    window.Swal.fire({
        ...
        html: `
            ...
            <div class="mb-3">
                ...
                <small class="text-muted">JSON format exports all selected data types</small>
            </div>
        `,
        showCancelButton: true,
        ...
        if (result.isConfirmed) {
            window.Swal.fire({
                ...
                html: '<p>Generating system data export...</p><p class="text-muted small">This may take a moment for large datasets</p>',
                allowOutsideClick: false,
                didOpen: () => {
                    window.Swal.showLoading();
