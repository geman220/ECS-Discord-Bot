...
class PassService:
    ...
    def generate_google_pass_url(self, wallet_pass: WalletPass) -> str:
        """
        ...
        if not GOOGLE_WALLET_AVAILABLE:
            raise ImportError("GoogleWalletPassGenerator library not installed")
    ...
    def generate_both_platforms(
        ...
        try:
            ...
            if apple_config.get('configured'):
                result['apple'] = self.generate_apple_pass(wallet_pass)
        ...
        try:
            ...
            if google_config.get('configured'):
                result['google'] = self.generate_google_pass_url(wallet_pass)
