...
def create_app(config_object='web_config.Config'):
    ...
    # Socket handlers - now a modular package
    # Old monolithic: from . import socket_handlers (from app/socket_handlers.py)
    # To rollback: rename app/sockets/ to app/sockets_new/ and rename app/socket_handlers_legacy.py to app/socket_handlers.py
...
def init_blueprints(app):
    ...
    """
    logger = logging.getLogger(__name__)
    # Auth - now a modular package
    # Old monolithic: from app.auth import auth as auth_bp (from app/auth.py)
    # To rollback: rename app/auth/ to app/auth_new/ and rename app/auth_legacy.py to app/auth.py
    ...
    # Old monolithic mobile API - DISABLED for new modular API migration
    # To rollback: uncomment the line below and comment out the init_mobile_api() call
    # from app.app_api import mobile_api
    # Old monolithic monitoring module - DISABLED for new modular package migration
    # To rollback: rename app/monitoring/ to app/monitoring_new/ and rename app/monitoring.py.bak to app/monitoring.py
    # from app.monitoring import monitoring_bp  # Old import from app/monitoring.py
    from app.monitoring import monitoring_bp  # Now imports from app/monitoring/__init__.py package
    ...
    # Old monolithic mobile API - DISABLED
    # To rollback: uncomment and comment out init_mobile_api() below
    # app.register_blueprint(mobile_api, url_prefix='/api/v1')

    # New modular mobile API (Phase 1 refactoring)
    # This is the refactored version of app_api.py split into focused modules
    # The old mobile_api from app/app_api.py is preserved but disabled
    # To rollback: comment out the lines below and uncomment mobile_api above
    ...
    # New modular monitoring package (Phase 2 refactoring)
    # Register all monitoring routes before adding blueprint
