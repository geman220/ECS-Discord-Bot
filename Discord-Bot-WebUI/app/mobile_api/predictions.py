...
def get_correct_predictions(match_id):
    ...
    try:
        with managed_session() as session:
            ...
            logger.info(f"Found {len(correct_users)} correct predictions for match {match_id}: {correct_users}")
        return jsonify({'correct_predictions': correct_users}), 200
    except Exception as e:
        logger.exception(f"Error fetching correct predictions for match {match_id}: {str(e)}")
