#!/usr/bin/env python3
"""
ESPN Player Service for Discord Bot
...
class ESPNPlayerService:
    ...
    async def get_player_image(self, player_name: str, team_name: str = "Seattle Sounders FC") -> Optional[str]:
        ...
        try:
            # Check cache first
            cache_key = f"player:{team_name}:{player_name.lower()}"
            ...
            if player_data:
                # Cache the result
                self._cache_player_data(cache_key, player_data)
    ...
    async def get_team_roster(self, team_name: str = "Seattle Sounders FC") -> Dict[str, Dict[str, Any]]:
        ...
        """
        try:
            cache_key = f"roster:{team_name.lower()}"
            cached_roster = self._get_from_cache(cache_key)
            ...
            if roster:
                # Cache for 24 hours
                self._cache_data(cache_key, roster)
    ...
    def _cache_player_data(self, key: str, player_data: Dict[str, Any]):
        """Cache player data."""
        self._cache_data(key, player_data)
