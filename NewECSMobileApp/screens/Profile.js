...
const goal_frequency_choices = [
    ...
    {
        label:
            "Only if our normal GK is unavailable and there are no other options",
        value: "1",
    },
    {
        label:
            "I will fill in as much as needed, but expect to play in the field",
        value: "2",
    },
...
// ----- Helper Functions -----
// Map a single stored value to its label using a provided choices array.
const mapValueToLabel = (choices, value) => {
    if (!value) return "N/A";
    const foundChoice = choices.find((choice) => choice.value === value);
    return foundChoice ? foundChoice.label : value;
};

// Map multiple stored values (string or array) to their labels.
const mapValuesToLabels = (choices, values) => {
    if (!values) return "N/A";
    let valuesArray = [];
    if (typeof values === "string") {
        valuesArray = values.replace(/[{}]/g, "").split(",").map((v) => v.trim());
    } else if (Array.isArray(values)) {
        valuesArray = values;
    }
    return valuesArray.map((val) => mapValueToLabel(choices, val)).join(", ");
};
...
const Profile = ({ navigation }) => {
    ...
    const [currentEditingPositions, setCurrentEditingPositions] = useState(null);
    ...
    const fetchProfileData = async () => {
        try {
            ...
            const profileResponse = await axios.get(
                `${globalConfig.API_URL}/user_profile?include_stats=true`,
                { headers }
            );
    ...
    const renderProfileItem = (label, value, key) => (
        <Block row space="between" style={styles.profileItem} key={key}>
            ...
            {editingFields[key] ? (
                ...
                <Text bold size={14} color="#525F7F">
                    {value}
                </Text>
            )}
            <TouchableOpacity
        ...
        <Block style={styles.fieldContainer}>
            ...
            {editingFields[field] ? (
                        ...
                        isMulti ? (
                            ...
                            <Picker
                                ...
                                onValueChange={(itemValue) =>
                                    setEditedData({ ...editedData, [field]: itemValue })
                                }
                            >
                                {options.map((option) => (
                ...
                <View style={styles.valueContainer}>
                    {/* Display the mapped label (or labels) instead of the raw value */}
                    <Text bold size={14} color="#525F7F">
                        {value}
                    </Text>
                    <TouchableOpacity
                        onPress={() => {
                            ...
                            // For editing, pass the raw value from profileData
                            setEditedData({ ...editedData, [field]: profileData[field] });
                        }}
    ...
    const renderProfileTab = () => (
        <Block>
            <Block style={styles.section}>
                <Text bold size={16} color="#525F7F" style={styles.sectionTitle}>
                ...
                {renderEditableField(
                    "Preferred Pronouns",
                    mapValueToLabel(pronoun_choices, profileData.pronouns),
                    "pronouns",
                    pronoun_choices
                )}
            </Block>
            ...
            <Block style={styles.section}>
                ...
                {renderEditableField(
                    "Favorite Position",
                    mapValueToLabel(soccer_positions, profileData.favorite_position),
                    "favorite_position",
                    soccer_positions
                )}
                {renderEditableField(
                    "Other Positions Enjoyed",
                    mapValuesToLabels(soccer_positions, profileData.other_positions),
                    "other_positions",
                    soccer_positions,
                    true
                )}
                {renderEditableField(
                    "Positions to Avoid",
                    mapValuesToLabels(soccer_positions, profileData.positions_not_to_play),
                    "positions_not_to_play",
                    soccer_positions,
                    true
                )}
                {renderEditableField(
                    "Jersey Size",
                    mapValueToLabel(jersey_sizes, profileData.jersey_size),
                    "jersey_size",
                    jersey_sizes
                )}
                ...
                {renderEditableField(
                    "Willing to Referee",
                    mapValueToLabel(willing_to_referee_choices, profileData.willing_to_referee),
                    "willing_to_referee",
                    willing_to_referee_choices
                )}
                {renderEditableField(
                    "Goal Frequency",
                    mapValueToLabel(goal_frequency_choices, profileData.frequency_play_goal),
                    "frequency_play_goal",
            ...
            <Block style={styles.section}>
                ...
                {renderEditableField(
                    "Available Weeks",
                    profileData.expected_weeks_available,
                    "expected_weeks_available",
            ...
            <Block flex middle style={styles.modalContainer}>
                <Block style={styles.modalContent}>
                    <Text bold size={18} style={styles.modalTitle}>
                        {currentEditingPositions === "other_positions"
                            ? "Other Positions"
                            : "Positions to Avoid"}
                    </Text>
                    <FlatList
                        data={
                            editedData[currentEditingPositions]
                                ? editedData[currentEditingPositions]
                                    .replace(/[{}]/g, "")
                                    .split(",")
                                : []
                        ...
                        renderItem={({ item }) => (
                            <Block row middle space="between" style={styles.positionItem}>
                                ...
                                <TouchableOpacity
                                    onPress={() => {
                                        const updatedPositions = editedData[
                                            currentEditingPositions
                                        ]
                                            .replace(/[{}]/g, "")
                                            .split(",")
                                            .filter((pos) => pos.trim() !== item.trim());
                                        setEditedData({
                                            ...editedData,
                                            [currentEditingPositions]: `{${updatedPositions.join(",")}}`,
                                        });
                                    }}
                                >
